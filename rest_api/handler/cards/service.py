...
from common.constants import (
    list_of_fetch_cards_cols,
    default_timelimits,
    admin_list_of_fetch_cards_cols,
)
...
def check_card_validity_and_update(card_id, answered):
    ...
    if not answered:
        if wrong_choice + 1 == 10:
            query = set_card_to_next_stage_query(
                card_id, current_stage, 10, int(time.time())
            )
        else:
            query = set_card_to_next_stage_query(
                card_id, 1, wrong_choice + 1, int(time.time()) + 5
            )

    if answered:
        if current_stage + 1 == 11:
            query = set_card_to_next_stage_query(
                card_id, 11, wrong_choice, int(time.time())
            )
        else:
            query = set_card_to_next_stage_query(
                card_id,
                current_stage + 1,
                wrong_choice,
                int(time.time()) + default_timelimits[current_stage],
            )
...
def get_card_or_cards(card_id=None):
    if card_id:
        ...
        card_exists, error = psql_instance.execute_fetch_query_single_value(
            card_exists_query
        )
        if error:
        ...
        if card_exists:
            ...
            db_response, error = psql_instance.execute_fetch_query_single_row(
                fetch_card_query
            )
            response = {}
            if not error:
        ...
        db_response, error = psql_instance.execute_fetch_query_for_all_data(
            fetch_all_cards_query
        )
        response = []
        if not error:
