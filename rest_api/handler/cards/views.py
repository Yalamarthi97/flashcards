...
class CardsV1(MethodView):
    ...
    def get(self):
        ...
        if data_exists:
            ...
            if db_not_empty:
                ...
                return {
                    "error": "There is no data in the data base",
                    "type": "text",
                }, 404
    ...
    def post(self, request_body):
        if_exists, check_error = check_card_exists(
            request_body["card_key"], request_body["card_desc"]
        )
        if check_error:
            return {
                "error": "Failed to check if the card is already present",
                "type": "text",
            }, 500
        if not if_exists:
            ...
            return {
                "error": "Card already exists with the same description",
                "type": "tex",
            }, 400
    ...
    def patch(self, request_body):
        ...
        if card_valid:
            if (
                str(request_body["answered"]) == "True"
                or str(request_body["answered"]) == "true"
            ):
                answered = True
            else:
                answered = False
            ...
            return {
                "message": "Card with this id is either complete or marked as forgotten , try resetting it",
                "type": "text",
            }, 200
...
class AdminCards(MethodView):
    ...
    def patch(self, request_body):
        if request_body["card_desc"]:
            card_exists, check_error = check_card_exists(
                request_body["card_key"], request_body["card_desc"]
            )
            ...
            if not card_exists:
                ...
                if not error:
                    return {
                        "message": "Updated the description of the card ",
                        "type": "text",
                    }, 200
                else:
                    return {"error": "Failed to update card", "type": "text"}, 500
                ...
                return {
                    "error": "Failed to update card as the pair of key and  description already exists",
                    "type": "text",
                }, 400
